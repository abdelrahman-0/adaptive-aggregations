...
template <custom_concepts::is_communication_page BufferPage>
class CircularEgressNetworkManager : public NetworkManager<BufferPage> {
    // needed for dependent lookups
    using NetworkManager<BufferPage>::ring;
    using NetworkManager<BufferPage>::free_pages;
    using NetworkManager<BufferPage>::buffers_start;
    std::vector<std::vector<BufferPage>> buffers;
    std::vector<bool> has_inflight;
    std::vector<BufferPage*> active_buffer;
    std::vector<u16> heads;
    std::vector<u16> tails;
    std::vector<io_uring_cqe*> cqes;
    u16 nbuffers;
    io_uring_cqe* cqe{nullptr};
  ...
  public:
    CircularEgressNetworkManager(u32 npeers, u32 nwdepth, u32 nbuffers, bool sqpoll, const std::vector<int>& sockets)
        : NetworkManager<BufferPage>(nwdepth, nbuffers, sqpoll, sockets), active_buffer(npeers, nullptr),
          has_inflight(npeers, false), cqes(nwdepth * 2), nbuffers(nbuffers), buffers(npeers), heads(npeers, 0),
          tails(npeers, 0) {
        for (auto& bufs : buffers) {
            bufs.resize(nbuffers);
        }
        for (auto peer{0u}; peer < npeers; ++peer) {
            get_new_page(peer);
        }
    ...
    [[maybe_unused]] BufferPage* get_new_page(u32 dst) {
        BufferPage* result{nullptr};
        if (tails[dst] - heads[dst] >= nbuffers) {
            int ret;
            if ((ret = io_uring_wait_cqe(&ring, &cqe)) < 0) {
                throw IOUringWaitError(ret);
            }
            if (cqe->res % sizeof(BufferPage) != 0) {
                throw IOUringSendError{cqe->res};
            }
            // TODO cqe encodes dst as well as num pages (and update head to npages-1)
            auto user_data = io_uring_cqe_get_data(cqe);
            // TODO need to peek cqe until we get dst == peeked_dst
            result = get_pointer<BufferPage>(user_data);
            io_uring_cq_advance(&ring, 1);
            // TODO not enough to flush last page
            auto peeked_dst = get_tag(user_data);
            if (tails[peeked_dst] - heads[peeked_dst] <= 1) {
                has_inflight[peeked_dst] = false;
            } else {
                _flush(peeked_dst, false);
                heads[peeked_dst]++;
            }
            result->clear_tuples();
            inflight_egress--;
            ...
            // get a free page
            result = buffers[dst][tails[dst]++ % nbuffers];
        }
        active_buffer[dst] = result;
        return result;
    }
    ...
    void _flush(u16 dst, bool flush_final) {
        // TODO wrap around
        // TODO encode dst and npages
        auto* sqe = io_uring_get_sqe(&ring);
        if (sqe == nullptr) {
            throw IOUringSubmissionQueueFullError{};
        }
