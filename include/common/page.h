...
// PAX-style page
template <uint64_t page_size, typename... Attributes>
struct alignas(page_size) Page {
    ...
    template <uint64_t other_page_size, typename... OtherAttributes>
    ...
    template <uint64_t other_page_size, typename... OtherAttributes, std::size_t... indexes>
...
template <uint64_t size, typename... Attributes>
    ...
    for (auto i{0u}; i < page.max_num_tuples_per_page; ++i) {
        if (std::get<0>(std::get<0>(page.columns)[i]) != 0 and std::get<0>(std::get<0>(page.columns)[i]) != '0' and
            ...
            println("weird character:", std::get<0>(std::get<0>(page.columns)[i]),
                    ...
                    uint64_t(*reinterpret_cast<const uint64_t*>(&std::get<0>(page.columns)[i])), "idx:", i, "/",
