...
template <IDX_MODE mode, typename Key, typename Value, concepts::is_mem_allocator Alloc, bool is_chained, bool use_ptr>
struct BaseAggregationHashtable {
  ...
  protected:
    ...
    void initialize(u64 size)
    {
        ...
        // alloc ht
        slots = Alloc::template alloc<idx_t>(sizeof(idx_t) * size);
    }
};
