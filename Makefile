...
build-debug:
	mkdir -p $(BUILD_DIR_DEB) && cd $(BUILD_DIR_DEB) && cmake -DCMAKE_BUILD_TYPE=Debug .. && make $(TARGETS)

build-release:
	mkdir -p $(BUILD_DIR_REL) && cd $(BUILD_DIR_REL) && cmake -DCMAKE_BUILD_TYPE=Release .. && make $(TARGETS)
