...
int main(int argc, char *argv[]) {
    ...
    for (auto thread_id{0u}; thread_id < FLAGS_threads; ++thread_id) {
        threads.emplace_back(
            [=, &local_node, &current_swip, &swips, &table, &pages_recv, &barrier_start, &barrier_end DEBUGGING(,
                ...
                mem::BlockAllocatorNonConc<NetworkPage> block_alloc{npeers * 3, FLAGS_maxalloc};
                ...
                manager_send.register_page_consumer_fn([&block_alloc](NetworkPage *page) { block_alloc.return_page(page); });
                ...
                for (u32 part{0}; part < FLAGS_partitions; ++part) {
                    ...
                    if (dst == node_id) {
                        ...
                        consumer_fns.emplace_back([&manager_send, actual_dst, final_dst_partition](NetworkPage *page, bool is_last = false) {
                ...
                auto process_local_page = [node_id, partition_mask, destinations, &partitions, &block_alloc, &consumer_fns](const PageTable &page) {
                ...
                while ((morsel_begin = current_swip.fetch_add(FLAGS_morselsz)) < swips.size()) {
                    ...
                    auto swizzled_idx =
                            std::stable_partition(swips.data() + morsel_begin, swips.data() + morsel_end,
                                                  [](const Swip &swip) { return !swip.is_pointer(); }) - swips.data();

                    // submit io requests before processing in-memory pages to overlap I/O with computation
                    thread_io.batch_async_io<READ>(table.segment_id, std::span{swips.begin() + morsel_begin, swips.begin() + swizzled_idx},
                                                   local_buffers, true);
