...
int main(int argc, char* argv[])
{
    ...
    for (u16 thread_id : range(FLAGS_threads)) {
        threads.emplace_back(
            [=, &local_node, &current_swip, &swips, &table, &storage_glob, &barrier_query, &barrier_preagg, &ht_glob, &sketch_glob, &times_preagg,
             &pages_pre_agg DEBUGGING(, &tuples_processed, &tuples_sent, &tuples_received, &pages_recv)]
            {
                ...
                if (thread_id == 0) {
                    if (FLAGS_consumepart) {
                        std::ranges::for_each(storage_glob.partition_pages.begin(), storage_glob.partition_pages.end(),
