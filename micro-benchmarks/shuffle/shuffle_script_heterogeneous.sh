TARGET_PATH='../../build-release/micro-benchmarks/shuffle/shuffle_heterogeneous'
LOCAL_NODE_ID="${NODE_ID:-0}"

MAX_NODES=4
FLAGS="--nolocal --random --npages=500000 --morselsz=10 --pin"
...
for NODES in $(seq $MAX_NODES -1 $(($LOCAL_NODE_ID + 1))); do
  for qthreads_per_nthread in 1 2 3; do
    for THREADS in 4 6 12 24 30 32; do
      ...
      for BUFS_PER_PEER in 4; do
        for TRY in $(seq 1 5); do
          if [[ "${LOCAL_NODE_ID}" == 0 ]]; then
            sleep 1s
          fi
          $TARGET_PATH $PRINT_HEADER $FLAGS --nodes=$NODES --nthreads=$NTHREADS --qthreads=$QTHREADS --bufs_per_peer=$BUFS_PER_PEER >/dev/null
          sleep 1s
