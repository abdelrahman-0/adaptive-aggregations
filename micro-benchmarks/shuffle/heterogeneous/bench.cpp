...
int main(int argc, char* argv[])
{
    ...
    if (FLAGS_random) {
        ...
        print("reading bytes:", offset_begin, "→", offset_end, (offset_end - offset_begin) / defaults::local_page_size, "pages");
    }
    ...
    for (auto thread_id{0u}; thread_id < FLAGS_nthreads; ++thread_id) {
        threads_network.emplace_back([=, &topology, &thread_grps, &barrier_network, &barrier_end, &nthread_continue DEBUGGING(, &pages_recv)]() {
            if (FLAGS_pin) {
            ...
            auto qthreads_per_nthread = (FLAGS_qthreads / FLAGS_nthreads) + (thread_id < (FLAGS_qthreads % FLAGS_nthreads));

            EgressManager manager_send{static_cast<u16>(npeers),         FLAGS_depthnw, npeers * FLAGS_bufs_per_peer * qthreads_per_nthread,
    ...
    for (auto thread_id{0u}; thread_id < FLAGS_qthreads; ++thread_id) {
        threads_query.emplace_back([=, &topology, &current_swip, &swips, &table, &barrier_start, &barrier_end, &added_last_page, &qthreads_done, &nthread_continue,
            ...
            while ((morsel_begin = current_swip.fetch_add(FLAGS_morselsz)) < swips.size()) {
                ...
                auto swizzled_idx = std::stable_partition(swips.data() + morsel_begin, swips.data() + morsel_end, [](const Swip& swip) { return !swip.is_pointer(); }) - swips.data();

                // submit io requests before processing in-memory pages to overlap I/O with computation
                if (swizzled_idx > morsel_begin) {
                    thread_io.batch_async_io<READ>(table.segment_id, std::span{swips.begin() + morsel_begin, swips.begin() + swizzled_idx}, local_buffers, true);
                }
